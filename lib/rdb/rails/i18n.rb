...
module I18nPluralizationLoader
  def init_translations(locale)
    super.tap do
      if RDB_PLURALIZATIONS.key?(locale)
        store_translations locale, RDB_PLURALIZATIONS[locale]
      else
        lang = locale.to_s.split('-', 2).first.to_sym
        if RDB_PLURALIZATIONS.key?(lang)
          store_translations locale, RDB_PLURALIZATIONS[lang]
        end
      end
    end
  end
